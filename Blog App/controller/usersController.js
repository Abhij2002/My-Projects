  ...
  try {
    ...
    {   
        if(await bcrypt.compare(password, user.password))
        {
            ...
            res.send(
                '<script>alert("Password Invalid"); window.location="/login";</script>'
              );
            }
          }
    }
      ...
      res.send(
          '<script>alert("User not Verified"); window.location="/login";</script>'
        );
      }
  } catch (error) {
    console.error(error);
...
module.exports.signup = async (req, res) => {
  ...
  if (username !== "" || password !== "" || mail !== "") {
    ...
    if (user) {
      res.send(
        '<script>alert("User Already Exist"); window.location="/login";</script>'
      );
    }
    else
    {
    ...
    res.send(
      '<script>alert("Enter Username or Password or E-mail"); window.location="/signup";</script>'
    );
  }
};
